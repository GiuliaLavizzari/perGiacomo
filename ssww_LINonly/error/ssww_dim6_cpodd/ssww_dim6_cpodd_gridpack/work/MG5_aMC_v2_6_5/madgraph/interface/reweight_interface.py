...
class ReweightInterface(extended_cmd.Cmd):
    ...
    def do_launch(self, line):
        ...
        if not self.has_standalone_dir:                           
            if self.rwgt_dir and os.path.exists(pjoin(self.rwgt_dir,'rw_me','rwgt.pkl')):
                ...
                if "FIRST_REWEIGHT_PATH" in os.environ:
                    copy_path = os.environ["FIRST_REWEIGHT_PATH"]
                else:
                    copy_path = "/tmp"
                cmd_logger.info('--> Creating first reweight point')
                self.first_path = pjoin(self.rwgt_dir,'rw_me')
                cmd_logger.info('--> Initial path: ' + self.first_path)
                self.first = False
                self.create_standalone_directory()
                os.system("cp -r {} {}".format(self.first_path, copy_path))
                self.first_path = pjoin(copy_path,'rw_me')
                self.compile()
                self.load_module()  
                ...
                cmd_logger.info('--> Copying base folder')
                os.system("cp -r {} {}".format(self.first_path, self.rwgt_dir))
                cmd_logger.info('--> Creating New ME folder')
                self.create_standalone_directory(second=True)
                cmd_logger.info('--> Compiling')
                self.compile()
                self.load_module()  
                if self.multicore == 'create':
                    self.load_module()
    ...
    def compile(self):
        ...
        for onedir in self.rwgt_dir_possibility:
            ...
            os.environ['MENUM'] = '2'
            os.chdir(pdir)
            os.system("sed -i.bak \'/FFLAGS= -O -w -fbounds-check -fPIC/s/$/ -mcmodel=medium/\' ../Source/make_opts")
